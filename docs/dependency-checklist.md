...
已声明（仓库根 `package.json`）：

1. `turbo`：用于 Monorepo 任务编排。
2. `husky`：用于把提交前前端 TypeScript 校验接入 Git Hook。

当前明确不接入：

1. `lint-staged`：当前先不引入。  
   原因：它更适合 staged 文件级 lint/格式化，而当前最需要兜住的是“后端契约变更后前端全量类型断裂”，这应由全量 `tsc -b` 负责。
...
5. 已补充 `frontend` 的 `typecheck` 脚本，用于提交前全量 TypeScript 校验。
